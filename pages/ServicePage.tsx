  ...
  if (!item) {
    return (
        <div className="container py-5 text-center mt-5">
            ...
            <Link to="/" className="text-brand-primary text-decoration-none fw-bold">Return Home</Link>
        </div>
    );
  }
           ...
           <div className="container">
              <div className="col-lg-8">
                  ...
                  {(item as any).subtitle && (
                    <Reveal delay={400}>
                      <p className="lead text-blue-100">{(item as any).subtitle}</p>
                    </Reveal>
                  )}
              </div>
           </div>
                ...
                <section>
                    ...
                    <div className="row g-3">
                        ...
                        <div className="col-md-6" key={idx}>
                            <Reveal delay={idx * 50}>
                                <div className="d-flex align-items-start gap-3 bg-white p-3 rounded-3 border border-light h-100 shadow-sm">
            ...
            <div className="d-flex flex-column gap-4">
                ...
                <Reveal delay={100}>
                    <div className="bg-white p-4 rounded-4 shadow-sm border border-light">
                        <h3 className="h5 fw-bold mb-3 text-brand-dark">Explore More</h3>
                        <div className="list-group list-group-flush">
                            {dataList.filter(i => i.slug !== slug).slice(0, 5).map(linkItem => (
                                <Link key={linkItem.slug} to={`/${type}s/${linkItem.slug}`} className="list-group-item list-group-item-action d-flex justify-content-between align-items-center px-0 text-secondary hover-text-primary border-light">
                                    {linkItem.title}
                                    <ArrowRight size={14} />
                                </Link>
                            ))}
